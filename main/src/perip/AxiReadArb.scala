...
class AxiReadArb(masterN: Int) extends Module {
  import AxiReadArb._

  val master = IO(Vec(masterN, Flipped(new ReadIO)))
  val slave = IO(new AxiReadIO)

  // 每个master是否正在进行请求
  val running = RegInit(VecInit(Seq.fill(masterN)(false.B)))
  // 每个master是否有待接收的请求
  val pending = Wire(Vec(masterN, Bool()))
  for (i <- 0 until masterN) pending(i) := !running(i) && master(i).req
  // 当前是否存在待接收的请求
  val doReq = WireDefault(pending.reduce(_ || _))
  // 若有请求待接收，应该是哪个id（id大优先级更高）
  val doReqId = Wire(UInt(4.W))
  doReqId := (0 until masterN).foldLeft(0.U)((res, i) => Mux(pending(i), i.U, res))
